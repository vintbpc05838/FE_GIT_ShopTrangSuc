...
const Customers = () => {
    ...
    const fetchCustomers = async () => {
      ...
      try {
        const response = await axios.get(
          "http://localhost:8080/api/admin/accounts/customer",
          {
  ...
  const getCustomers = async () => {
    const token = localStorage.getItem("token");
    try {
        const response = await fetch("http://localhost:8080/api/admin/accounts/customer", {
            method: "GET",
            headers: {
        ...
        customers.forEach((customer) => {
            console.log("Địa chỉ mặc định của khách hàng:", customer.defaultAddress);
        });
    } catch (error) {
...
//       customers.forEach((customer) => {
//           // Kiểm tra và log địa chỉ mặc định của khách hàng
//           if (customer.defaultAddress) {
//               console.log("Địa chỉ mặc định của khách hàng:", customer.defaultAddress);
//           } else {
//               console.log("Khách hàng này chưa có địa chỉ mặc định.");
//           }
//       });
//   } catch (error) {
//       console.error("Lỗi khi lấy danh sách khách hàng:", error);
            ...
            <div className="customer-details-customer">
              <div className="detail-section-customer">
                ...
                <p>
                  ...
                  <em>{selectedCustomer.fullname}</em>
                </p>
                <p>
              ...
              <div className="detail-section-customer">
                <p>
                  <strong>Giới tính:</strong> <em>{selectedCustomer.gender}</em>
                </p>
                <p>
                ...
                <p>
                  ...
                  <em>
                    ...
                    selectedCustomer.addresses.length > 0
                      ? `${selectedCustomer.addresses[0].address}, ${selectedCustomer.addresses[0].district}, ${selectedCustomer.addresses[0].city}`
                      : "Chưa cập nhật"}
                  </em>
                </p>
              </div>
